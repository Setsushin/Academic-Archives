...
parser.add_argument('--exp-number', type=int, default=2,
                    help='1:DNN, 2:BNN, 3:MLUTNET, 4:B-MLUTNET')
parser.add_argument('--dataset', type=str, default='STL10',
                    help='dataset choice (STL-10)')

parser.add_argument('--input-size', type=int, default=3*96*96,
...
if __name__ == "__main__":
    ...
    now = datetime.now()
    exp = 'runs_STL10/' + now.strftime('%Y-%m-%d') + '/' + args.dataset + '/' + model.__name__ + '_' + args.LR_mode
    ...
    if args.cuda:
        dummy_input = torch.rand(3, 96, 96).sign().cuda()
    else:   
        dummy_input = torch.rand(3, 96, 96).sign()
    writer.add_graph(model, (dummy_input,))

    # Draw confusion matrix 绘制混淆矩阵
    train_set = datasets.STL10('../data', 'test', transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize((0.1307,), (0.3081,))]))
    ...
    cm = confusion_matrix(train_set.labels, train_preds.argmax(dim=1))
